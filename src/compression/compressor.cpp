...
int NormalizeZstdLevel(int requestedLevel) {
...
std::vector<uint8_t> CompressBuffer(const std::vector<uint8_t>& data,
    ...
    case CompressionAlgorithm::Zstd: {
        ...
        const int normalizedLevel = NormalizeZstdLevel(zstdLevel);
        ...
        if (context != nullptr) {
            size_t status = ZSTD_CCtx_setParameter(context, ZSTD_c_compressionLevel, normalizedLevel);
            bool advancedModeAvailable = ZSTD_isError(status) == 0;
#ifdef ZSTD_c_enableLongDistanceMatching
            if (advancedModeAvailable && normalizedLevel >= 15 && data.size() >= (4u << 20)) {
                status = ZSTD_CCtx_setParameter(context, ZSTD_c_enableLongDistanceMatching, 1);
                advancedModeAvailable = ZSTD_isError(status) == 0;
            }
#endif
#ifdef ZSTD_c_windowLog
            if (advancedModeAvailable && normalizedLevel >= 15 && data.size() >= (4u << 20)) {
                status = ZSTD_CCtx_setParameter(context, ZSTD_c_windowLog, 27);
                advancedModeAvailable = ZSTD_isError(status) == 0;
            }
#endif
            if (advancedModeAvailable) {
                written = ZSTD_compress2(context, compressed.data(), compressed.size(), data.data(), data.size());
            } else {
                written = ZSTD_compress(compressed.data(), compressed.size(), data.data(), data.size(), normalizedLevel);
            }
