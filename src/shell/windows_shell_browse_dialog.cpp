...
void ExecuteBrowseExtractToFolder(BrowseDialogState& state, HWND window) {
    ...
    const fs::path destinationRoot = fs::path(selectedFolder);
    const auto overwriteCallback =
        [window, destinationRoot](const std::string& existingPath, const std::string& archiveEntryPath) {
            return PromptForOverwriteDecision(window, destinationRoot, fs::u8path(existingPath), archiveEntryPath);
        };
...
void ExecuteBrowseExtractHere(BrowseDialogState& state, HWND window) {
    ...
    const fs::path destinationRoot = fs::u8path(state.defaultDestination);
    const auto overwriteCallback =
        [window, destinationRoot](const std::string& existingPath, const std::string& archiveEntryPath) {
            return PromptForOverwriteDecision(window, destinationRoot, fs::u8path(existingPath), archiveEntryPath);
        };
