...
namespace zipbox::shell::internal {
...
void CreateBrowseMenu(HWND window) {
    ...
    AppendMenuW(helpMenu, MF_STRING, kBrowseMenuHelpAbout, L"About ZipBox");
...
void ExecuteBrowseExtractToFolder(BrowseDialogState& state, HWND window) {
    ...
    ProgressDialog progressDialog(L"ZipBox - Extracting archive");
    const auto progressCallback = [&progressDialog](const zipbox::utils::ProgressInfo& info) {
        return progressDialog.Update(info);
    };
    zipbox::extraction::ExtractArchive(
        state.archivePath,
        fs::path(selectedFolder).u8string(),
        state.password,
        progressCallback);
    MessageBoxW(window, L"Extraction completed.", L"ZipBox", MB_OK | MB_ICONINFORMATION);
}
...
void ExecuteBrowseTest(BrowseDialogState& state, HWND window) {
    zipbox::extraction::TestArchive(state.archivePath, state.password);
    MessageBoxW(window, L"Archive passed integrity checks.", L"ZipBox", MB_OK | MB_ICONINFORMATION);
}
...
void OpenSelectedEntry(BrowseDialogState& state, HWND window) {
    ...
    if (selectedIndex < 0 || static_cast<size_t>(selectedIndex) >= state.entries.size()) {
        MessageBoxW(window, L"Select a file first.", L"ZipBox", MB_OK | MB_ICONINFORMATION);
        return;
    }

    const auto& selected = state.entries[static_cast<size_t>(selectedIndex)];
    const std::vector<uint8_t> plain = zipbox::extraction::ReadArchiveEntry(
        state.archivePath,
        static_cast<size_t>(selectedIndex),
        state.password);

    fs::path outputPath = fs::temp_directory_path() / "ZipBoxPreview" / fs::path(selected.path).filename();
    ...
    case WM_COMMAND:
        ...
        if (LOWORD(wParam) == IDOK) {
            ...
            if (state->password.empty()) {
                MessageBoxW(window, L"Password is required.", L"ZipBox", MB_OK | MB_ICONERROR);
                return 0;
            }
...
std::wstring BuildBrowseLine(const zipbox::archive::ArchiveEntryInfo& entry) {
...
void LoadBrowseEntries(BrowseDialogState& state) {
    ...
    const zipbox::archive::ArchiveMetadata metadata = zipbox::extraction::GetArchiveMetadata(state.archivePath);
    if (!metadata.encrypted) {
        state.password.clear();
        state.entries = zipbox::extraction::ListArchiveEntries(state.archivePath);
    } else {
        while (true) {
            ...
            try {
                state.entries = zipbox::extraction::ListArchiveEntries(state.archivePath, candidatePassword);
                state.password = std::move(candidatePassword);
                break;
            } catch (const std::exception& error) {
                const std::wstring message = Utf8ToWide(std::string(error.what()) + "\n\nTry again?");
                if (MessageBoxW(state.window, message.c_str(), L"ZipBox", MB_YESNO | MB_ICONERROR) != IDYES) {
                    throw std::runtime_error("Password entry canceled");
                }
            }
        }
    ...
    case kBrowseInitializeMessage:
        ...
        try {
            ...
            if (std::string(error.what()) != "Password entry canceled") {
                MessageBoxW(window, Utf8ToWide(error.what()).c_str(), L"ZipBox", MB_OK | MB_ICONERROR);
            }
            state->done = true;
            DestroyWindow(window);
        }
...
bool PromptForArchivePassword(HWND owner, const std::string& archivePath, std::string& passwordOut) {
    ...
    const wchar_t* className = L"ZipBoxPasswordPrompt";
    ...
    if (!classRegistered) {
        ...
        if (RegisterClassExW(&windowClass) == 0) {
            throw std::runtime_error("Failed to register the ZipBox password prompt");
        }
    ...
    HWND window = CreateWindowExW(
        WS_EX_DLGMODALFRAME,
        className,
        L"ZipBox - Password Required",
        WS_CAPTION | WS_SYSMENU | WS_POPUP | WS_VISIBLE,
        CW_USEDEFAULT, CW_USEDEFAULT, 388, 152,
        owner, nullptr, GetModuleHandleW(nullptr), &state);

    if (window == nullptr) {
        throw std::runtime_error("Failed to create the ZipBox password prompt");
    }
...
void ShowBrowseDialog(const std::string& archivePath) {
    const wchar_t* className = L"ZipBoxBrowseDialog";
    static bool classRegistered = false;
    ...
    if (!classRegistered) {
        ...
        windowClass.lpszClassName = className;
        if (RegisterClassExW(&windowClass) == 0) {
            throw std::runtime_error("Failed to register the ZipBox browser window class");
        }
        classRegistered = true;
    }
    ...
    HWND window = CreateWindowExW(
        WS_EX_DLGMODALFRAME,
        className,
        Utf8ToWide("ZipBox - " + fs::path(archivePath).filename().u8string()).c_str(),
        WS_CAPTION | WS_SYSMENU | WS_POPUP | WS_VISIBLE,
        CW_USEDEFAULT, CW_USEDEFAULT, 670, 390,
        nullptr, nullptr, GetModuleHandleW(nullptr), &state);

    if (window == nullptr) {
        throw std::runtime_error("Failed to create the ZipBox browser window");
    }
