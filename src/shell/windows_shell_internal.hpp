...
struct ShellAddDialogResult {
    ...
    zipbox::compression::CompressionAlgorithm algorithm = zipbox::compression::CompressionAlgorithm::Zstd;
    ShellSpeedPreset speedPreset = ShellSpeedPreset::Normal;
    int zstdLevel = 8;
    std::string password;
    zipbox::crypto::EncryptionAlgorithm encryption = zipbox::crypto::EncryptionAlgorithm::Aes256;
    size_t splitSize = 0;
};
...
public:
    explicit ProgressDialog(const std::wstring& title) {
        ...
        dialog_->SetCancelMsg(L"Canceling ZipBox operation...", nullptr);
    ...
    bool Update(const zipbox::utils::ProgressInfo& info) {
