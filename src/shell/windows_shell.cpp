...
bool TryAcquireArchivePassword(const std::string& archivePath, HWND owner, std::string& password) {
    if (!zipbox::archive::LooksLikeZoxArchive(archivePath)) {
        password.clear();
        return true;
    }

    const zipbox::archive::ArchiveMetadata metadata = zipbox::archive::ReadArchiveMetadata(archivePath);
...
void ExecuteShellExtraction(const std::string& archivePath,
    ...
    zipbox::extraction::ExtractArchive(archivePath, destination, password, progressCallback);
}

} // namespace zipbox::shell::internal

namespace zipbox::shell {
...
bool RunShellAddDialog(const std::vector<std::string>& targetPaths) {
    ...
    internal::ProgressDialog progressDialog(L"ZipBox - Creating archive");
    const auto progressCallback = [&progressDialog](const zipbox::utils::ProgressInfo& info) {
        return progressDialog.Update(info);
    };

    if (dialogResult.format == internal::ShellArchiveFormat::Zip) {
        zipbox::archive::CreateZipArchive(targetPaths, internal::fs::path(dialogResult.outputPath).u8string(), config, progressCallback);
    } else {
        zipbox::archive::CreateArchive(
            targetPaths,
            [&]() {
                internal::fs::path outputPath(dialogResult.outputPath);
                if (zipbox::utils::ToLower(outputPath.extension().u8string()) == ".zox") {
...
void RunQuickAddZox(const std::vector<std::string>& targetPaths) {
    ...
    zipbox::archive::CreateArchive(targetPaths, internal::DefaultZoxBasePath(targets), config, progressCallback);
}
