...
void PopulateCompressionCombo(HWND combo,
                              ...
                              zipbox::compression::CompressionAlgorithm preferred) {
    SendMessageW(combo, CB_RESETCONTENT, 0, 0);

    if (format == ShellArchiveFormat::Zip) {
        AddComboEntry(combo, L"zlib", static_cast<LPARAM>(zipbox::compression::CompressionAlgorithm::Zlib));
        AddComboEntry(combo, L"store", static_cast<LPARAM>(zipbox::compression::CompressionAlgorithm::Store));
    } else {
        AddComboEntry(combo, L"zstd", static_cast<LPARAM>(zipbox::compression::CompressionAlgorithm::Zstd));
        AddComboEntry(combo, L"zlib", static_cast<LPARAM>(zipbox::compression::CompressionAlgorithm::Zlib));
        AddComboEntry(combo, L"store", static_cast<LPARAM>(zipbox::compression::CompressionAlgorithm::Store));
    }
    ...
    int selectedIndex = 0;
    for (int index = 0; index < count; ++index) {
        if (static_cast<zipbox::compression::CompressionAlgorithm>(
...
bool IsZstdCompressionSelected(HWND combo) {
    return static_cast<zipbox::compression::CompressionAlgorithm>(GetSelectedComboValue(combo)) ==
        zipbox::compression::CompressionAlgorithm::Zstd;
}
...
void RefreshArchiveDialog(ShellAddDialogState& state) {
    ...
    const auto preferred = format == ShellArchiveFormat::Zip
        ? zipbox::compression::CompressionAlgorithm::Zlib
        : zipbox::compression::CompressionAlgorithm::Zstd;
...
bool BrowseArchivePath(ShellAddDialogState& state) {
    ...
    const wchar_t* filter = format == ShellArchiveFormat::Zip
        ...
        : L"ZipBox archive (*.zox)\0*.zox\0\0";
    ...
    dialog.lpstrTitle = L"ZipBox - Select archive destination";
...
LRESULT CALLBACK ShellAddDialogProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        AddComboEntry(state->encryptionCombo, L"AES256", static_cast<LPARAM>(zipbox::crypto::EncryptionAlgorithm::Aes256));
        AddComboEntry(state->encryptionCombo, L"Gorgon", static_cast<LPARAM>(zipbox::crypto::EncryptionAlgorithm::Gorgon));
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            try {
                ...
                state->result.algorithm = static_cast<zipbox::compression::CompressionAlgorithm>(
                ...
                state->result.zstdLevel = state->result.algorithm == zipbox::compression::CompressionAlgorithm::Zstd &&
                ...
                if (state->result.format == ShellArchiveFormat::Zox) {
                    ...
                    state->result.encryption = static_cast<zipbox::crypto::EncryptionAlgorithm>(
                    ...
                    state->result.encryption = zipbox::crypto::EncryptionAlgorithm::None;
...
ShellAddDialogResult ShowShellAddDialog(const std::vector<fs::path>& targets) {
    const wchar_t* className = L"ZipBoxShellAddDialog";
    ...
    if (!classRegistered) {
        ...
        if (RegisterClassExW(&windowClass) == 0) {
            throw std::runtime_error("Failed to register the ZipBox dialog window class");
        }
