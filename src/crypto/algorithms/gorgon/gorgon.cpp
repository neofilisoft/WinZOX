...
std::vector<uint8_t> EncryptGorgon(const std::vector<uint8_t>& plainText,
    ...
    const std::vector<unsigned char> aesKey = DeriveKey(password, metadata.salt, metadata.iterations);
    const std::vector<unsigned char> serpentKey = DeriveKey(password, SecondarySalt(metadata.salt), metadata.iterations);
