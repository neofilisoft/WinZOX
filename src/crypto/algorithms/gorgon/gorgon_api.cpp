...
#define WINZOX_GORGON_API_VERSION "2.11.1"
#endif
...
void SecureWipeVector(std::vector<uint8_t>& buffer) {
    if (!buffer.empty()) {
...
WinZOXGorgonStatus winzox_gorgon_encrypt(const uint8_t* plain_data,
    ...
    try {
        plain = CopyInputBytes(plain_data, plain_size);
        const auto& provider = winzox::crypto::GetGorgonProvider();
        cipher = provider.Encrypt(
            plain,
            passwordCopy,
            ToInternalMetadata(*metadata));
        SetError(error_buffer, error_buffer_size, "");
        const WinZOXGorgonStatus status = ExportBuffer(cipher, out_cipher, error_buffer, error_buffer_size);
        SecureWipeVector(cipher);
        SecureWipeVector(plain);
...
WinZOXGorgonStatus winzox_gorgon_decrypt(const uint8_t* cipher_data,
    ...
    try {
        cipher = CopyInputBytes(cipher_data, cipher_size);
        const auto& provider = winzox::crypto::GetGorgonProvider();
        plain = provider.Decrypt(
            cipher,
            passwordCopy,
            ToInternalMetadata(*metadata),
            expected_plain_size);
        SetError(error_buffer, error_buffer_size, "");
        const WinZOXGorgonStatus status = ExportBuffer(plain, out_plain, error_buffer, error_buffer_size);
        SecureWipeVector(plain);
        SecureWipeVector(cipher);
        SecureWipeString(passwordCopy);
        return status;
