...
fs::path ResolveOutputPathForWrite(const fs::path& destinationRoot,
    ...
    while (fs::exists(candidatePath) && !replaceAll) {
        ...
        case OverwriteAction::Rename: {
            ...
            candidatePath = winzox::utils::EnsurePathWithinRoot(destinationRoot, renamedPath, decision.renamedPath);
            break;
        }
        }
    }
...
void ExtractGenericArchive(const std::string& filename,
    ...
    while ((result = archive_read_next_header(reader, &entry)) == ARCHIVE_OK) {
        ...
        if ((entryFileType & AE_IFLNK) == AE_IFLNK ||
            archive_entry_symlink(entry) != nullptr ||
            archive_entry_hardlink(entry) != nullptr) {
            archive_write_free(writer);
            archive_read_free(reader);
            throw std::runtime_error("Archive links are blocked during extraction: " + currentFile);
        }
